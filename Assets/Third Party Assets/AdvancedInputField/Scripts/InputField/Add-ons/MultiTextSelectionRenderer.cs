...
namespace AdvancedInputFieldPlugin
{
	...
	[ExecuteInEditMode]
	public class MultiTextSelectionRenderer: MaskableGraphic
	{
		public struct TextSelectionRegion
		{
			...
			public TextSelectionRegion(int startPosition, int endPosition)
			{
		...
		private List<TextSelectionRegion> selectionRegions;
		...
		protected override void Awake()
		{
			...
			selectionRegions = new List<TextSelectionRegion>();
		}
		...
		internal void UpdateSelectionRegions(List<TextSelectionRegion> selectionRegions)
		{
		...
		protected override void OnPopulateMesh(VertexHelper vertexHelper)
		{
			...
			if(selectionRegions == null) { return; }

			int length = selectionRegions.Count;
			for(int i = 0; i < length; i++)
			{
				DrawTextSelectionRegion(vertexHelper, selectionRegions[i]);
			}
		}

		private void DrawTextSelectionRegion(VertexHelper vertexHelper, TextSelectionRegion region)
		{
			int startPosition = region.startPosition;
			int endPosition = region.endPosition;
